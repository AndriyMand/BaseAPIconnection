...
class CategoryTreeAdminList extends CategoryTreeAbstract
{
    ...
    public function getCategoryOptionList(array $categories_array, int $repeat = 0)
    {
        foreach ($categories_array as $value)
        {
            $this->categorylistOptions[] = ['name'=> str_repeat("-",$repeat).$value['name'], 'id'=>$value['id']];
            
            if(!empty($value['children'])) 
            {
                $repeat = $repeat + 2;
                $this->getCategoryOptionList($value['children'],$repeat);
                $repeat = $repeat - 2;
            }

        }
        return $this->categorylistOptions;
    }
